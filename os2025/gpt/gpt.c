...
static float *global_out;
static float *global_inp;
static float *global_weight;
static float *global_bias;
static int global_B, global_T, global_C, global_OC;
static const int global_n_threads = 16;
int flag = 1;
sem_t begin[16];
sem_t finish[16];
...
/*void worker(int id) {
    while (1) {
        ...
        int tid = id - 1;
        int total_tasks = global_B * global_T;
        int chunk_size = total_tasks / global_n_threads;
        int start = tid * chunk_size;
        int end = (tid == global_n_threads - 1) ? total_tasks : start + chunk_size;

        for (int k = start; k < end; k++) {
            int b = k / global_T;
            int t = k % global_T;

            float* out_bt = global_out + b * global_T * global_OC + t * global_OC;
            float* inp_bt = global_inp + b * global_T * global_C + t * global_C;

            if (global_bias) {
                for (int o = 0; o < global_OC; o++) {
                    float val = global_bias[o];
                    float* wrow = global_weight + o * global_C;
...
void worker(int id) {
    while (1) {
        ...
        const int t_stride = 4;
        
        const int b_offset = (tid >> 2) & 0x03;
        const int t_offset = tid & 0x03;

        for (int b = b_offset; b < global_B; b += b_stride) {
            for (int t = t_offset; t < global_T; t += t_stride) {
                float* out_bt = global_out + b * global_T * global_OC + t * global_OC;
                float* inp_bt = global_inp + b * global_T * global_C + t * global_C;
